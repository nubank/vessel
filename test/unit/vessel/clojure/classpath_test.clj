...
(deftest assemble-deps-test
  (testing "returns all expected files"
    (is (match? (m/in-any-order ["data.json-0.2.6.jar"
                ...
                (map #(.getName %) (classpath/assemble-deps {:clojure/tool :tools.deps.alpha :project/descriptor (io/file project "deps.edn")})))))

  (testing "returns the same set of files for Leiningen and tools.deps.alpha"
    (is (match? (m/in-any-order  (classpath/assemble-deps {:clojure/tool :leiningen :project/descriptor (io/file project "project.clj")}))
                (classpath/assemble-deps {:clojure/tool :tools.deps.alpha :project/descriptor (io/file project "deps.edn")})))))
