...
(defn- read-image-manifest
  ...
  [^File source-dir]
  (first
   (json/read-str (slurp (io/file source-dir "manifest.json"))
                  :key-fn (comp keyword misc/kebab-case))))
...
(defn push
  ...
  (let [_                                 (jib.helpers/extract-tarball tarball temp-dir)
        {:keys [config repo-tags layers] :as m}
        (read-image-manifest temp-dir)
        ^ImageReference image-reference   (ImageReference/parse (first repo-tags))
