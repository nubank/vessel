(ns vessel.clojure.classpath
  "Agnostic classpath assembler built on top of clojure.tools.deps.alpha.
...
(defmulti ^{:doc "Assembles classpaths for projects managed by tools.deps and/or
Leiningen in an uniform way.

Takes a map containing the keys :clojure/tool (nowadays, :leiningen or
:tools.deps) and :project/file (a java.io.File) and returns a sequence of
java.io.File objects describing the project's classpath. The classpath only
contains project dependencies since root directories such as src and resources
must be supplied as inputs for Vessel."}  assemble :clojure/tool)

(s/fdef assemble
  :args (s/cat :options (s/keys :req [:clojure/tool :project/file]))
  :ret (s/coll-of v1/file?))

(defmethod assemble :leiningen
  [{:project/keys [file]}]
  (-> file
...
(defmethod assemble :tools.deps
  [{:project/keys [file]}]
  (make-classpath (misc/read-edn file)))
